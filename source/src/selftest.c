...
int run_selftests(bool verbose){
    ...
    for(int k=0;k<3;k++){memset(b,0xcc,n);uint64_t v=stream_kernel(ks[k],a,b,n);check(v==scalar_xor(a,n,rr[k]),"Mixed kernel read subset checksum");bool good=true;for(size_t i=0;i<n/8;i++){uint64_t want=((i*8)%256)>=(size_t)rr[k]*64?write_value():0xccccccccccccccccULL;if(((uint64_t*)b)[i]!=want){good=false;break;}}check(good,"Mixed kernel exact write subset and untouched addresses");}
    ...
    check(kernel_bytes(K_READ,n)==n,"Read byte accounting");check(kernel_bytes(K_WRITE_NT,n)==n,"Write byte accounting");check(kernel_bytes(K_COPY_NT,n)==n*2,"Copy read-plus-write byte accounting");check(kernel_bytes(K_TURN_SWEEP,n)==n*2,"Turnaround sweep read-plus-write byte accounting");
    check(kernel_bytes(K_ALT50,n)==kernel_bytes(K_GROUP50,n),"Grouped and alternating workloads count equal logical bytes");check(kernel_bytes(K_MIX75,n)==n&&kernel_bytes(K_MIX25,n)==n,"Mixed workloads count requested logical bytes, not assumed bus traffic");
    check(turnaround_events(64,64)==1,"One 64B read/write group has one direction transition");check(turnaround_events(128,64)==3,"Two 64B read/write groups have three direction transitions");check(turnaround_events(128,128)==1,"One 128B read/write group has one direction transition");
    const size_t cn=256*1024;for(int mode=0;mode<3;mode++)for(unsigned k=1;k<=8;k*=2){Chain c;chain_build(&c,a,cn,k,mode,17492);check(chain_check(&c),"Disjoint pointer cycles cover the full working set");Node* h[8]={0};memcpy(h,c.heads,sizeof(h));chase_batch(h,k,c.count/k);bool same=true;for(unsigned j=0;j<k;j++)if(h[j]!=c.heads[j])same=false;check(same,"Pointer kernel counts loads and returns after a full cycle");}
    ...
    check(fabs(42.0*2000.0/8400.0-10.0)<1e-12,"DDR5 8400 CL42 converts to 10 ns (theory only)");check(fabs(1073741824.0/0.5/1e9-2.147483648)<1e-12,"Throughput uses measured bytes/seconds and decimal GB/s");
    Profile p={0};check(profile_get(&p,"missing")==NULL,"Missing profile keys return NULL instead of a synthetic value");
