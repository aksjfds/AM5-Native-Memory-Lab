...
int run_selftests(bool verbose){
    ...
    check(kernel_bytes(K_READ,n)==n,"Read byte accounting");check(kernel_bytes(K_WRITE_NT,n)==n,"Write byte accounting");check(kernel_bytes(K_COPY_NT,n)==n*2,"Copy read-plus-write byte accounting");check(kernel_bytes(K_ALT50,n)==kernel_bytes(K_GROUP50,n),"Grouped and alternating workloads count equal logical bytes");check(kernel_bytes(K_MIX75,n)==n&&kernel_bytes(K_MIX25,n)==n,"Mixed workloads count requested logical bytes, not assumed bus traffic");
