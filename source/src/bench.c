...
typedef struct {
    Shared* shared;Core core;uint8_t *a,*b;size_t n,pattern_bytes;Kernel kernel;
    uint64_t bytes,passes,events,sink,errors,expected;double finished;bool pinned;Thread thread;
} Worker;
...
static Worker* workers_launch(Report* r,Shared* s,uint8_t* a,uint8_t* b,size_t bytes,Kernel k,int threads,int first,size_t pattern_bytes){
    ...
    for(int i=0;i<threads;i++){size_t begin=blocks*(size_t)i/(size_t)threads*4096,end=blocks*(size_t)(i+1)/(size_t)threads*4096;
        ...
        if(w[i].n<4096)die("Too many threads for the working set.");if(k==K_READ||k==K_TURN_SWEEP)w[i].expected=xor_words(w[i].a,w[i].n);
...
static Sample bandwidth_ex(Report* r,uint8_t* a,uint8_t* b,size_t n,Kernel k,int threads,const char* suite,const char* test,int trial,size_t pattern_bytes){
    Sample x={0};fmt(x.suite,sizeof(x.suite),"%s",suite);fmt(x.test,sizeof(x.test),"%s",test?test:kernel_id(k));fmt(x.unit,sizeof(x.unit),"GB/s");x.trial=trial;x.threads=threads;x.working_bytes=n;x.pattern_bytes=pattern_bytes;x.pin_ok=true;
    Shared s;Worker* w=workers_launch(r,&s,a,b,n,k,threads,0,pattern_bytes);gate_start(&s,r->opt.seconds);double end=s.start;
    for(int i=0;i<threads;i++){thread_join(w[i].thread);if(w[i].finished>end)end=w[i].finished;x.logical_bytes+=w[i].bytes;x.operations+=w[i].passes;x.events+=w[i].events;x.pin_ok=x.pin_ok&&w[i].pinned;x.errors+=w[i].errors;}
...
static Sample bandwidth(Report* r,uint8_t* a,uint8_t* b,size_t n,Kernel k,int threads,const char* suite,int trial){return bandwidth_ex(r,a,b,n,k,threads,suite,0,trial,0);}
...
static void finish_windows(Sample* x,double* windows,size_t stored){qsort(windows,stored,sizeof(double),doublecmp);x->p50=quantile_sorted(windows,stored,.5);x->p95=quantile_sorted(windows,stored,.95);x->p99=quantile_sorted(windows,stored,.99);x->p999=quantile_sorted(windows,stored,.999);x->max_value=stored?windows[stored-1]:0;}
static Sample latency_measure(Report* r,Chain* c,const char* suite,const char* test,int trial,double deadline){
    ...
    while(!atomic_load(&cancelled)&&now_sec()<deadline){double a=now_sec();chase_batch(h,c->chains,rounds);double b=now_sec();double dt=b-a;if(dt<=0)continue;x.operations+=(uint64_t)rounds*c->chains;x.elapsed+=dt;double ns=dt*1e9/((double)rounds*c->chains);seen++;if(stored<8192)windows[stored++]=ns;else {size_t j=(size_t)(rng_next(&seed)%seen);if(j<8192)windows[j]=ns;}}
    x.value=x.operations?x.elapsed*1e9/(double)x.operations:0;finish_windows(&x,windows,stored);free(windows);uint64_t sink=0;for(unsigned i=0;i<c->chains;i++)sink^=(uintptr_t)h[i];atomic_fetch_xor(&global_sink,sink);return x;
}
static Sample stall_probe_measure(Report* r,Chain* c,const char* suite,const char* test,int trial,double deadline){
    Sample x={0};fmt(x.suite,sizeof(x.suite),"%s",suite);fmt(x.test,sizeof(x.test),"%s",test);fmt(x.unit,sizeof(x.unit),"ns/access");x.trial=trial;x.threads=1;x.chains=1;x.working_bytes=c->bytes;x.pattern_bytes=64;x.pin_ok=pin_core(r->hw.cores[0]);
    Node* h[8]={c->heads[0]};chase_batch(h,1,8192);const size_t cap=32768,batch=64;double *windows=malloc(cap*sizeof(double));if(!windows)die("Cannot allocate stall probe samples.");size_t stored=0,seen=0;uint64_t seed=0x5e1187b93ULL;if(deadline<=0)deadline=now_sec()+r->opt.seconds;
    while(!atomic_load(&cancelled)&&now_sec()<deadline){double a=now_sec();chase_batch(h,1,batch);double b=now_sec();double dt=b-a;if(dt<=0)continue;double ns=dt*1e9/(double)batch;x.operations+=batch;x.elapsed+=dt;seen++;if(stored<cap)windows[stored++]=ns;else {size_t j=(size_t)(rng_next(&seed)%seen);if(j<cap)windows[j]=ns;}}
...
static Sample loaded(Report* r,Chain* c,uint8_t* a,uint8_t* b,size_t bytes,Kernel k,int n,int trial){
    ...
    pin_core(r->hw.cores[0]);Node* warm[8]={0};memcpy(warm,c->heads,sizeof(warm));chase_batch(warm,1,8192);Shared s;Worker* w=workers_launch(r,&s,a,b,bytes,k,n,1,0);gate_start(&s,r->opt.seconds);
...
static Sample loaded_stall_copy(Report* r,Chain* c,uint8_t* a,uint8_t* b,size_t bytes,int n,int trial){
    ...
    pin_core(r->hw.cores[0]);Node* warm[8]={c->heads[0]};chase_batch(warm,1,8192);Shared s;Worker* w=workers_launch(r,&s,a,b,bytes,K_COPY_NT,n,1,0);gate_start(&s,r->opt.seconds);
...
void record_sample(Report* r,const Sample* x){
    if(r->nsamples>=MAX_RESULTS)die("Result limit exceeded.");r->samples[r->nsamples++]=*x;char path[2400];fmt(path,sizeof(path),"%s/raw.csv",r->outdir);FILE* f=file_open(path,r->nsamples==1?"wb":"ab");if(!f)die("Cannot write raw.csv; extract the program to a writable folder.");
    ...
    fprintf(f,"%s,%s,%d,%d,%d,%.0f,%.0f,%.0f,%.0f,%.0f,%.9f,%.9f,%s,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%.0f\n",x->suite,x->test,x->trial,x->threads,x->chains,(double)x->working_bytes,(double)x->pattern_bytes,(double)x->operations,(double)x->events,(double)x->logical_bytes,x->elapsed,x->value,x->unit,x->p50,x->p95,x->p99,x->p999,x->max_value,x->background_gbps,x->pin_ok,(double)x->errors);fclose(f);
...
static bool job_exists(Job* jobs,int n,Kernel k,int t){for(int i=0;i<n;i++)if(jobs[i].k==k&&jobs[i].t==t)return true;return false;}
static void add_job(Job* jobs,int* n,Kernel k,int t){if(t>0&&!job_exists(jobs,*n,k,t))jobs[(*n)++]=(Job){k,t};}
void suite_run(Report* r){
    ...
    printf("\n[2/7] Copy baseline, isolated read/write ceilings, and Copy thread scaling\n");fflush(0);Job jobs[32];int nj=0;int counts[5]={1,2,4,8,nt};
    for(int i=0;i<5;i++)if(counts[i]<=nt)add_job(jobs,&nj,K_COPY_NT,counts[i]);add_job(jobs,&nj,K_READ,1);add_job(jobs,&nj,K_READ,nt);add_job(jobs,&nj,K_WRITE_NT,1);add_job(jobs,&nj,K_WRITE_NT,nt);add_job(jobs,&nj,K_COPY_CACHED,nt);
    ...
    printf("\n[6/7] Random latency under read, mixed, and Copy background traffic\n");fflush(0);if(!atomic_load(&cancelled)){Chain c;chain_build(&c,cp,n,1,0,0x719ff02);int bg=nt>1?nt-1:0;Job lj[12];int nl=0;lj[nl++]=(Job){K_READ,0};if(bg>0){lj[nl++]=(Job){K_READ,bg};lj[nl++]=(Job){K_MIX50,bg};int cc[4]={1,2,4,bg};for(int i=0;i<4;i++)if(cc[i]<=bg)add_job(lj,&nl,K_COPY_NT,cc[i]);}
        for(int rep=1;rep<=r->opt.repeats&&!atomic_load(&cancelled);rep++){int order[12];for(int i=0;i<nl;i++)order[i]=i;permute_int(order,nl,&seed);for(int j=0;j<nl&&!atomic_load(&cancelled);j++){Job job=lj[order[j]];Sample s=loaded(r,&c,a,b,n,job.k,job.t,rep);record_sample(r,&s);if(s.errors)atomic_store(&cancelled,true);}}}
    report_write(r);
