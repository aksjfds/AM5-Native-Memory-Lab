...
static Sample stall_probe(Report* r,Chain* c,int trial){
    Sample x={0};fmt(x.suite,sizeof(x.suite),"stall_probe");fmt(x.test,sizeof(x.test),"short_window_random");fmt(x.unit,sizeof(x.unit),"ns/access");x.trial=trial;x.threads=1;x.chains=1;x.working_bytes=c->bytes;x.pattern_bytes=64;x.pin_ok=pin_core(r->hw.cores[0]);
    Node* h[8]={c->heads[0]};chase_batch(h,1,8192);const size_t cap=32768,batch=64;double *windows=malloc(cap*sizeof(double));if(!windows)die("Cannot allocate stall probe samples.");size_t stored=0,seen=0;uint64_t seed=0x5e1187b93ULL;double deadline=now_sec()+r->opt.seconds;
    while(!atomic_load(&cancelled)&&now_sec()<deadline){double a=now_sec();chase_batch(h,1,batch);double b=now_sec();double dt=b-a;if(dt<=0)continue;double ns=dt*1e9/(double)batch;x.operations+=batch;x.elapsed+=dt;seen++;
        if(stored<cap)windows[stored++]=ns;else {size_t j=(size_t)(rng_next(&seed)%seen);if(j<cap)windows[j]=ns;}}
...
void suite_run(Report* r){
    ...
    printf("\n[1/7] Allocate and pre-fault arrays; basic full-buffer integrity checks\n");fflush(0);
    ...
    printf("\n[2/7] Real read/write/copy, thread scaling, and read/write access patterns\n");fflush(0);
    ...
    report_write(r);
    printf("\n[3/7] Read/write turnaround sweep: fixed 50/50 bytes, varied direction-switch spacing\n");fflush(0);
    const size_t groups[6]={64,256,1024,4096,16384,65536};
    for(int rep=1;rep<=r->opt.repeats&&!atomic_load(&cancelled);rep++){int order[6]={0,1,2,3,4,5};permute_int(order,6,&seed);
        for(int j=0;j<6&&!atomic_load(&cancelled);j++){size_t g=groups[order[j]];char name[64];fmt(name,sizeof(name),"rw_group_%lluB",(unsigned long long)g);Sample s=bandwidth_ex(r,a,b,n,K_TURN_SWEEP,nt,"turnaround_sweep",name,rep,g);record_sample(r,&s);if(s.errors)atomic_store(&cancelled,true);}}
    report_write(r);
    printf("\n[4/7] Cache-sized working sets (single core; not AIDA64 aggregate scores)\n");fflush(0);
    ...
    report_write(r);
    printf("\n[5/7] Serial random/page-local/sequential chains and 2/4/8 independent chains\n");fflush(0);
    ...
    report_write(r);
    printf("\n[6/7] Short-window random-latency tail probe (refresh candidate; not refresh attribution)\n");fflush(0);
    if(!atomic_load(&cancelled)){Chain c;chain_build(&c,cp,n,1,0,0x3190ab27);for(int rep=1;rep<=r->opt.repeats&&!atomic_load(&cancelled);rep++){Sample s=stall_probe(r,&c,rep);record_sample(r,&s);}}
    report_write(r);
    printf("\n[7/7] Loaded latency: pointer chasing on one core + disjoint traffic on other cores\n");fflush(0);
