...
void report_write(const Report* r){
    Text j;text_init(&j);text_add(&j,"{\n\"schema\":\"AM5Native/2\",\"engine\":\"" ENGINE_ID "\",\"platform\":");
#ifdef _WIN32
    ...
    text_fmt(&j,"\"profile_loaded\":%s,\"profile\":{",r->profile.loaded?"true":"false");for(size_t i=0;i<r->profile.n;i++){if(i)text_add(&j,",");json_str(&j,r->profile.kv[i].key);text_add(&j,":");json_str(&j,r->profile.kv[i].value);}text_add(&j,"},\n\"samples\":[\n");
    for(size_t i=0;i<r->nsamples;i++){const Sample* x=&r->samples[i];if(i)text_add(&j,",\n");text_add(&j,"{\"suite\":");json_str(&j,x->suite);text_add(&j,",\"test\":");json_str(&j,x->test);text_add(&j,",\"unit\":");json_str(&j,x->unit);
        text_fmt(&j,",\"trial\":%d,\"threads\":%d,\"chains\":%d,\"working_bytes\":%.0f,\"operations\":%.0f,\"logical_bytes\":%.0f,\"elapsed\":%.12g,\"value\":%.12g,\"p50\":%.12g,\"p95\":%.12g,\"p99\":%.12g,\"background_gbps\":%.12g,\"pin_ok\":%s,\"errors\":%.0f}",x->trial,x->threads,x->chains,(double)x->working_bytes,(double)x->operations,(double)x->logical_bytes,x->elapsed,x->value,x->p50,x->p95,x->p99,x->background_gbps,x->pin_ok?"true":"false",(double)x->errors);
