...
static int program_main(int argc,char** argv){
    ...
    if(r->hw.available&&((uint64_t)r->opt.memory*3+64*MIB)>r->hw.available*3/4)die("Insufficient free RAM for a low-paging run. Close memory-intensive apps.");
    ...
    char parent[2048],tag[128];if(*r->opt.out)fmt(parent,sizeof(parent),"%s",r->opt.out);else fmt(parent,sizeof(parent),"%s/results",base);if(!make_dir(parent))die("Cannot create results folder.");stamp(tag,sizeof(tag),true);fmt(r->outdir,sizeof(r->outdir),"%s/%s",parent,tag);if(!make_dir(r->outdir))die("Cannot create session output folder.");
    ...
    printf("AM5 Native Memory Lab " VERSION " - Copy Bottleneck Analyzer\nCPU: %s\nThreads: %d | Per-array memory: %.0f MiB | Repeats: %d | Sample: %.0f ms\nOutput: %s\n\nGoal: locate the measured path/parameter group most likely limiting this program's Copy workload.\nThe program does not modify BIOS settings. Press Ctrl+C to stop safely.\n",r->hw.name,r->opt.threads,(double)r->opt.memory/MIB,r->opt.repeats,r->opt.seconds*1000,r->outdir);fflush(0);
...
#ifdef _WIN32
void mainCRTStartup(void){int argc=0;wchar_t** wa=CommandLineToArgvW(GetCommandLineW(),&argc);if(!wa||argc<1)ExitProcess(1);char** av=calloc((size_t)argc+1,sizeof(char*));if(!av)ExitProcess(1);for(int i=0;i<argc;i++){int n=WideCharToMultiByte(65001,0,wa[i],-1,0,0,0,0);if(n<1)ExitProcess(1);av[i]=malloc((size_t)n);if(!av[i])ExitProcess(1);WideCharToMultiByte(65001,0,wa[i],-1,av[i],n,0,0);}LocalFree(wa);int rc=program_main(argc,av);for(int i=0;i<argc;i++)free(av[i]);free(av);ExitProcess((unsigned)rc);}
#else
