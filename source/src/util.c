...
void text_fmt(Text* t,const char* f,...){va_list a,b;va_start(a,f);va_copy(b,a);int n=vlength(f,b);va_end(b);if(n<0)die("Formatting failure.");reserve(t,(size_t)n);vformat(t->data+t->len,t->cap-t->len,f,a);va_end(a);t->len+=(size_t)n;}
void json_str(Text* t,const char* s){text_add(t,"\"");for(const unsigned char* p=(const unsigned char*)s;*p;p++){unsigned c=*p;if(c=='"'||c=='\\')text_fmt(t,"\\%c",c);else if(c<32||c=='<'||c=='>'||c=='&')text_fmt(t,"\\u%04x",c);else {char x[2]={(char)c,0};text_add(t,x);}}text_add(t,"\"");}
