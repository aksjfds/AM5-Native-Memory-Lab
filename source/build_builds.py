...
src=['main','platform','util','kernels','bench','selftest','report']
def run(args):
 r=subprocess.run(args,cwd=P,text=True,capture_output=True)
 if r.stdout: print(r.stdout)
 if r.stderr: print(r.stderr)
 if r.returncode: raise RuntimeError('Failed: '+' '.join(args))
run(['clang','-std=c11','-O3','-Wall','-Wextra','-Wpedantic','-Wno-overlength-strings','-pthread',*[f'src/{s}.c' for s in src],'-lm','-o',str(B/'am5lab-linux')])
mods={
